...
def _build_data(G, labels, W):
    ...
    # Preserve raw scalar weights so the loss/architecture can access them
    # even after GatedGCN overwrites edge_attr with learned embeddings.
    return Data(x=x, edge_index=edge_index, edge_attr=edge_attr,
                edge_weight=edge_attr.clone(), y=y)


def generate_weighted_gp_instance(n=100, graph_type="er", eps=0.05):
...
class WeightedGPDataset(SyntheticDataset):
    """Mixed weighted graph-partitioning dataset (k=2).
    ...
    Data fields per graph:
      ...
      edge_weight : (2|E|, 1) clone of edge_attr (preserved across GNN layers)
      y           : (n,)     ground-truth balanced binary partition
    """

    def __init__(self, name, root, transform=None, pre_transform=None):
        super().__init__('weighted_gp', name, root, transform, pre_transform)
    ...
    def create_graph(self, idx):
        ...
        return generate_weighted_gp_instance(
            n=self.params.n,
