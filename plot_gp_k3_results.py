"""
plot_gp_k3_results.py

1. Training loss (GCON vs ScatteringClique) on one plot.
2. Validation loss (GCON vs ScatteringClique) on another plot.
3. 2x2 test-graph figure:
      top-left    : input graph (no partition)
      top-right   : spectral baseline
      bottom-left : GCON solution
      bottom-right: ScatteringClique solution

Usage:
    python plot_gp_k3_results.py
    python plot_gp_k3_results.py --graph_idx 3
"""
...
COLORS = ["#4878CF", "#D65F5F", "#6ACC65"]   # one per partition
...
def plot_loss_curves(gcon_csv, hybrid_csv):
    ...
    for col, ylabel, title, fname in [
        ("loss/train", "Training loss",   "GP k=3 — Training Loss",   "gp_k3_train_loss.png"),
        ("loss/valid", "Validation loss", "GP k=3 — Validation Loss", "gp_k3_val_loss.png"),
    ]:
...
def get_gp_solution(ckpt_path, cfg_path, graph_idx):
    """Returns (data, G, pos, logits, labels) for one test graph."""
    ...
    # prefer raw logits if saved by Softmax activation
    logits = getattr(batch, "logits", None)
    if logits is None:
        logits = batch.x
    logits = logits.squeeze().cpu().numpy()   # (n, 3)
...
def visualise_combined(gcon_ckpt, gcon_cfg, hybrid_ckpt, hybrid_cfg, graph_idx, k=3):
    ...
    fig.suptitle(
        f"Graph Partition (k=3) — test graph {graph_idx}  "
    ...
    draw_partition(axes[0][1], G, pos, spectral_labels,
                   "Spectral Baseline", spectral_cf, k)

    draw_partition(axes[1][0], G, pos, gcon_labels,
                   "GCON", gcon_cf, k)

    draw_partition(axes[1][1], G, pos, hybrid_labels,
                   "ScatteringClique", hybrid_cf, k)

    plt.tight_layout()
    out = "gp_k3_solutions.png"
...
def main():
    ...
    gcon_csv   = latest_file("results/gp_sbm_small_balanced_k3-*/**/metrics.csv")
    hybrid_csv = latest_file("results/gp_sbm_small_balanced_k3_hybridconv-*/**/metrics.csv")
    gcon_ckpt  = latest_file("results/gp_sbm_small_balanced_k3-*/**/*.ckpt")
    hybrid_ckpt= latest_file("results/gp_sbm_small_balanced_k3_hybridconv-*/**/*.ckpt")

    gcon_cfg   = "configs/benchmarks/gp/gp_sbm_small_balanced_k3.yaml"
    hybrid_cfg = "configs/benchmarks/gp/gp_sbm_small_balanced_k3_hybridconv.yaml"
